...
case class JudgeNote(hotelId: String, platform: String
                     , userId: String, userName: String
                     , userLevel: String, judgeTimes: Int
                     , coverCity: Int
                     , title: String
                     , scoreSum: Double
                     , scoreDtl1: Double
                     , scoreDtl2: Double
                     , scoreDtl3: Double, scoreDtl4: Double
                     , liveDate: String
...
object MockJudgeData {
  ...
  def mkRandomJude() = {
    ...
    val hotelId = random.nextInt(20) + 1 //hotelId 1-20
    val platform = platforms(random.nextInt(5))
    val userId = userIds(random.nextInt(100))
    val userLevel = userLevels(random.nextInt(4))
    val judgeTimes = random.nextInt(20) //评论次数 0-20
    val coverCity = random.nextInt(15) //遍布城市 0-15
    val title = "title"
    val scoreSum = random.nextInt(5) + 1
    val scoreDtl1 = random.nextInt(5) + 1
    val scoreDtl2 = random.nextInt(5) + 1
    val scoreDtl3 = random.nextInt(5) + 1
    val scoreDtl4 = random.nextInt(5) + 1
    ...
    JudgeNote(
      ...
      , judgeTimes
      , coverCity
      , title, scoreSum.toDouble
      , scoreDtl1.toDouble
      , scoreDtl2.toDouble
      , scoreDtl3.toDouble
      , scoreDtl4.toDouble
      , liveDate
      , isPlateformOrdered.toString
      , content)
  }
